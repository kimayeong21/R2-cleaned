...
기초 Python 프로젝트부터 데이터 분석, TCP 통신, 로봇 관제 GUI, Arduino 기반 안전 모듈, ROS2와 Spring Boot를 연동한 공기청정 로봇 프로젝트까지 직접 구현한 결과물을 프로젝트별로 정리했습니다. 각 프로젝트는 학습한 기술을 실제 동작하는 형태로 연결해보는 데 초점을 두었습니다.
...
여섯 번째 프로젝트는 `공기질 연동 자율주행 공기청정 로봇`입니다. 미세먼지 센서 데이터를 기반으로 오염 구역을 감지하고, TurtleBot3가 해당 구역으로 이동해 공기청정 동작을 수행하는 시스템입니다. ROS2 로봇 제어, MQTT 통신, Spring Boot 웹 대시보드, MySQL 데이터 저장 흐름을 함께 다뤘습니다.
